...
|--------------------------------------------------------------------------
| 1、匿名函数
| 2、字符串, 例如: Home\RouteController@miss
*/
Route::miss(function() {
...
| 
| 1、使用 name 数组键指定 名称路由
|
| 2、在路由定义后方链式调用 name 方法 
|
...
| 指定命名空间
|--------------------------------------------------------------------------
| 你可以使用 namespace 参数来指定相同的 PHP 命名空间给控制器群组
|
| 默认根命名空间为app\Http\Controllers, 你可以指定根命名空间之后的部分命名空间, 
| 开头结尾不需要加'\'
*/
...
*/
Route::group(['domain' => '{account}.onion.com'], function() {
...
| 路由模型绑定
|--------------------------------------------------------------------------
| 例如: 我们绑定 {_user} 参数至 app\Models\User 模型. 假如 请求地址为 model/1 时,
| 则会向方法 注入 ID为1 的表记录 (数组类型).
|
|
| 除了以下例子, 你还可以在 Onion\Providers\RouteServiceProvider::boot 方法中
| 定义模型绑定
|
| class RouteServiceProvider extends ServiceProvider {
|   public function boot(Router $router) {
...
Route::get('model/customize/{_user_customize}', function($_user_customize) {
	...
	if ($_user_customize !== false) return $_user_customize;
});
...
| CSRF 保护
|--------------------------------------------------------------------------
| 程序会为每个用户的 Session 自动生成一个 CSRF token , 用于保护应用程序不受
| 跨网站请求伪造 攻击. 非 GET 请求时, 需要传递 _token 参数
|
| 有时候你可能会希望一组 URL 不要被 CSRF 保护, 可以在 VerifyCsrfToken 中间件中
| 增加 $except 属性来排除 URL
*/
...
Route::match(['get', 'post'], 'csrf', 'Home\RouteController@csrf');
...
<<<EOF
	路由分组公共属性 (Route::group函数第一个参数) 支持以下设置:
	...
	uses:       路由操作			匿名函数、 字符串形式: Controller@action
EOF;
/*
|--------------------------------------------------------------------------
| 路由分组演示
